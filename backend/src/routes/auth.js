...
router.post("/login", async (req, res, next) => {
    try {
        ...
        if (!username || !password) return res.status(401).send({ type: "error", message: "falta dados"});
        const bdUser = await userSchema.findOne({ username });
        req.user = bdUser;
        if (!bdUser) return res.status(401).send({ type: "error", message: "dados inválidos" });
        const check = await bcrypt.compare(password, bdUser.password);
        if (!check) return res.status(401).send({ type: "error", message: "senha inválida" });
        else return next();
    } catch (error) {
